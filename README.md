...
### Final Results — 402 / 402 cells (v4 closed 2026-06-01)
...
**v4 scope added 2026-06-01** (PRD v4 closes the project): 30 Phase B2 (THz-protocol simplification, sat=0 + noise=0) + 6 Phase B2-nr (L3 no-regularization arm) + 90 Phase C2 (single-axis isolation under the THz protocol) + 48 multi-seed audit replicates. Total canonical: 402; total runs (including multi-seed): 450. Multi-seed L3 [mean ± std] aggregates live in `artifacts/Final_Exp.json` (`schema_version=3`). See [Findings #5–#8](#headline-research-findings) and `Final_Report.pdf` §VIII–§XIII for the headline analysis.
...
5. **Protocol simplification recovers ~1/3 of the v2 collapse — Phase B2.** Removing color saturation and additive Gaussian noise (the two THz-irrelevant axes) recovers ~3.24 pp on average against the Phase D T3 baseline — about 4× the +0.77 pp T3-only recovery from Finding #4. Combined Δ_B1→B2 = +4.01 pp across 30 cells (5 levels × 3 models × 2 datasets) closes 32% of the v2 −12.49 pp collapse. The Phase B2-nr L3 arm decomposes the recovery: Δ_B1→B2nr = +4.19 pp under no T3 regularization, and Δ_B2nr→B2 = +1.39 pp pure T3 contribution at the simpler protocol — T3 is ~80% more effective when the protocol is simpler (vs +0.77 pp T3 effect at the full v2 protocol). Operator-locked answer: **Most of the Phase B2 improvement is pure protocol simplification rather than additional regularization benefit.**

6. **Resolution dominates the THz axis-attribution budget by ~4× — Phase C2.** Under the Phase B2 / THz-simplified protocol (sat=0, noise=0), resolution accounts for −27.45 pp mean drop across L1–L5, blur −6.58 pp, and salt-and-pepper −5.13 pp. The ordering is consistent across all six (model, dataset) panels. Sum-of-3-axes is near-additive at L1–L4 (within 1–3 pp of combined Phase B2) but breaks at L5 with a 15.2 pp saturation effect (the 3×3-downsample bottleneck destroys spatial structure faster than the other two axes can additively degrade). MNIST geometric-axis floor carries forward: blur and salt-and-pepper drop MNIST by <0.2 pp at every level; only resolution hurts MNIST (~15 pp mean). CIFAR-10 absorbs the full hit: resolution −37 to −41 pp, blur −9 to −16 pp, salt-and-pepper −5 to −14 pp.

7. **Headline accuracies are stable across seeds; held-out test confirms val.** All 24 L3 base tags satisfy the operator-locked variance threshold of σ < 1.5 pp across the multi-seed audit at seeds {42, 43, 44}; worst case is σ ≈ 1.18 pp on TransNeXt-tiny D-T3 CIFAR-10. None of the Δ_B1→B2 / Δ_D→B2 / Δ_B1→B2nr / Δ_B2nr→B2 conclusions reverse sign under any seed. Held-out test split (seed=99 stratified, ~1000 per class) confirms val_acc curves with the val/test gap under 2 pp across the 72 evaluations.

8. **Calibration is the second-order signal of the information bottleneck.** Expected Calibration Error (ECE, n_bins=15) is bounded under 5 pp at L3 across the matrix (well-calibrated) and rises above 15 pp at L5 on CIFAR-10: the model retains high confidence as spatial information collapses, signaling the bottleneck is not a noise problem the model could shrug off but an information-loss problem the model cannot self-detect. Phase B2 calibration is 1–2 pp ECE better than Phase B1 at L3 — a quiet additional benefit of the protocol simplification. Per-image inference throughput on the RTX 5070 places all three architectures within a narrow few-ms band at batch_size=1 / bf16-mixed; accuracy under the THz protocol (Finding #5–#6) is the more discriminative criterion than throughput for THz-edge deployment.

### Recent scientific changes

- **2026-06-01 — v4 full scientific-depth scope closed (402 canonical cells + 48 multi-seed audit).** PRD v4 (`PHASE_B2` branch) added (i) **Phase B2** — a 30-cell THz-protocol simplification sweep stripping color saturation and additive Gaussian noise from the v2 pipeline (US-039 pilot, US-040 sweep); (ii) **Phase B2-nr** — a 6-cell L3 no-regularization arm decomposing the Δ_B1→B2 attribution (US-041); (iii) **Phase C2** — a 90-cell single-axis isolation sweep under the THz protocol attributing the dominant axis (US-043 pilot, US-044 sweep); (iv) **multi-seed L3 audit** — 48 runs across the 24 L3 headline cells at seeds 43 and 44 (US-042); (v) **diagnostics** — held-out test-set inference, L5 confusion matrices, L3+L5 calibration / ECE, RTX 5070 throughput (US-045); (vi) **`Final_Report.pdf` rev3** — six new appendices §VIII–§XIII spanning Phase B2, Phase C2, multi-seed variance, test-set confirmation, diagnostics, and Final Conclusions. Total v4 GPU-h spend ~122 GPU-h (vs ~100 GPU-h budget), 0 §6.3 sentinels across all 402 canonical cells, 0 of 24 multi-seed base tags exceeding the 1.5 pp std threshold. New operator-locked findings #5–#8 (above) attribute the Phase B2 recovery to ~3/4 protocol simplification + ~1/4 regularization benefit, attribute the Phase C2 drop to resolution dominating by ~4×, confirm seed stability and test-set generalization, and identify L5 calibration as the second-order signal of the information bottleneck. Source-of-truth: [PRD.md](PRD.md) US-036..US-053, [`docs/phase_b2.md`](docs/phase_b2.md), [`docs/phase_c2.md`](docs/phase_c2.md), [`Final_Exp.md`](Final_Exp.md) 402 rows.
