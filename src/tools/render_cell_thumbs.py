...
Sample index is fixed per dataset (val index 0) so the same source image
is used across every cell of that dataset, making visual comparison easy.
...
SAMPLE_INDEX_PER_DATASET = 0  # val[0] of each dataset, fixed across cells
...
def _build_pair(spec) -> torch.Tensor:
    ...
    img, _ = ds[SAMPLE_INDEX_PER_DATASET]
    ...
    seed = SAMPLE_INDEX_PER_DATASET + SEED_OFFSET_VAL
