...
\textbf{Inputs.}\quad Standard image datasets (CIFAR-10 [7]
and MNIST [8]), automatically downloaded by torchvision on
...
\input{tables/tab_II_levels.tex}

\textbf{Data pipeline.}\quad Pipeline version 2 (finalized 2026-05-20):
the saturation lerp is applied first, then the bilinear
...
\begin{enumerate}
  \item \textbf{TransNeXt variant: small to tiny.} The preliminary spec
        proposed \texttt{transnext\_small} ($\sim 50$M parameters); the
        2026-05-14 revision substituted \texttt{transnext\_tiny}
        ($\sim 28$M parameters) to keep the parameter count comparable to ResNet50
  ...
  \item \textbf{Multi-seed audit.} The preliminary spec used a single seed
        (42). The 2026-06-01 revision added a 48-replicate audit at
        seeds 43 and 44 across the 24 L3 headline cells so that the published L3
