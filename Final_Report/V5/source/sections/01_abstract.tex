...
downsample, blur, additive noise, salt and pepper). A staged design
isolates the effects, from clean baselines through combined and
single-axis degradation to a THz-relevant protocol subset; this report
analyses the combined-degradation phase (B1) and the single-axis THz
phase (C2) in depth and summarizes the rest. A 48-replicate multi-seed
audit on the 24 L3 headline cells bounds reproducibility. Every
degradation pipeline is anchored to a deterministic 256-image PSNR / SSIM
probe so the comparison sits on a physical scale.
