...
convolutional network (ResNet50 [1]), a densely connected
convolutional network with explicit feature reuse (DenseNet121
[2]), and a vision transformer with foveal attention
(TransNeXt-tiny [3]).
...
[4], [5], which inflates PSNR / SSIM scores without
...
in modern robustness benchmarks [6] and the one
...
\ref{sec:phases} defines the experimental phases at a glance. Section
\ref{sec:spec} reports the as-built technical specification and lists every
drift from the preliminary specification. Section \ref{sec:design} documents
the engineering approach and the mathematical model. Section \ref{sec:tests}
reports the experimental results: the two phases that carry the headline
science --- Phase B1 and Phase C2 --- are analysed in full with tables,
figures, and the per-model cross-comparison of Section \ref{sec:crossmodel},
while the remaining phases are summarized qualitatively. Section
\ref{sec:problems} enumerates the engineering and scientific problems
encountered during the 402-cell campaign and how each was resolved. Section
\ref{sec:conclusions} states the scientific conclusions, evaluates the project
against its preliminary goals, and compares the planned schedule and budget
with the actual execution. Section \ref{sec:references} lists the references.
